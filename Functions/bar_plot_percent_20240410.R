...
bar_plot_percent <- function(dataframe, x_var, y_var = "perc", fill_var, plot_name = "default_plot.png", facet_var = NULL, main_title = NULL, x_axis_label = NULL, y_axis_label = "Pct Selected", y_limits = c(0, 1)) {
  ...
  x_var_ordered <- reorder(stringr::str_wrap(!!as.name(x_var), 10), desc(!!as.name(y_var)))
  
  # Create a ggplot
  p <- ggplot(dataframe, aes(x = x_var_ordered, y = !!as.name(y_var), fill = factor(!!as.name(fill_var)))) +
    geom_bar(stat = "identity", width = 0.4) +
    geom_text(stat = 'identity', aes(label = glue::glue("{round(!!as.name(y_var) * 100)}%")), vjust = 1, size = 3, colour = "white") +
...
data <- data.frame(
  ...
  Value = c("X", "Y", "X", "Z", "Y", "Z"),
  Percentage = c(0.2, 0.3, 0.1, 0.4, 0.25, 0.75)
)

test_that("bar_plot_percent generates a valid plot", {
  # Generate the plot
  plot <- bar_plot_percent(data, "Variable", "Percentage", "Value", "test_plot.png")
...
data <- data.frame(
  ...
  Percentage = c(20, 30, 10, 40, 25, 75, 15, 85)
)

test_that("bar_plot_percent_ordered generates a valid plot", {
  # Generate the plot
  plot <- bar_plot_percent_ordered(data, "Category", "Percentage", "Category", "test_plot.png")
...
bar_plot_percent <- function(dataframe, x_var, y_var = "Percentage", fill_var, plot_name = "default_plot.png", facet_var = NULL, main_title = NULL, x_axis_label = NULL, y_axis_label = "Pct Selected", y_limits = c(0, 1)) {
  ...
  # Create a ggplot
  p <- ggplot(dataframe, aes(stringr::str_wrap(.data[[x_var]], 10), y = .data[[y_var]], fill = factor(.data[[fill_var]]))) +
    geom_bar(stat = "identity", width = 0.4) +
    geom_text(stat = 'identity', aes(label = glue::glue("{round(.data[[y_var]] * 100)}%")), vjust = 1, size = 3, colour = "white") +
    scale_fill_manual(values = rep(custom_palette, length.out = n_distinct(dataframe[[fill_var]]))) +
    expand_limits(y = y_limits) + # Adjust y-axis limits
...
data <- data.frame(
  Category = c("A", "A", "B", "B", "C", "C", "D", "D"),
  Group = c("X", "X", "X", "Y", "Y", "Y", "Z", "Z"),
  Percentage = c(20, 30, 10, 40, 25, 75, 15, 85)
)

test_that("bar_plot_percent generates a valid plot", {
  # Generate the plot
  plot <- bar_plot_percent(data, "Category", "Percentage", "Group", "test_plot.png")
