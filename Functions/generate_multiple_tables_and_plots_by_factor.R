...
bar_plot_percent_ordered <- function(dataframe, x_var, y_var = "Percentage", fill_var, plot_name = "default_plot.png", facet_var = "factor_variable", main_title = NULL, x_axis_label = NULL, y_axis_label = "Pct Selected", y_limits = c(0, 1)) {
...
# Helper function to generate and display the descriptive statistics for a given question

generate_question_descriptives <- function(data, question, file_name) {
  st_results <- st(data, vars = c(question), group = "factor_variable")
  print(st_results)
  write.csv(st_results, file = file_name, row.names = FALSE)
}


generate_multiple_tables_and_plots_by_factor <- function(df_large, factor_variable, questions) {
  for (question in questions) {
    # Select the relevant columns from the large dataframe
    df <- dplyr::select(df_large, response_id, !!sym(question), !!sym(factor_variable))
    
    # Convert the question variable to a factor
    df[[question]] <- as.factor(df[[question]])
    
    # Ensure the factor variable is a factor variable
    df[[factor_variable]] <- as.factor(df[[factor_variable]])
    
    # Process the data
    question_df_perc <- process_data_pipeline_multiple_columns(df, id.var = c("response_id", factor_variable), default_value = 0, group_column = c(factor_variable, "value"))
    
    # Generate the bar plot
    bar_plot_percent_ordered(question_df_perc, "value", "new_perc", "value", paste0(question, "_DF_PERC.png"),
                             facet_var = factor_variable, main_title = "", x_axis_label = " ", y_axis_label = "Pct. Selected", y_limits = c(0, 0.25))
    ...
    generate_question_descriptives(df, question, paste0(question, "_descriptives.csv"))
  }
}
...
DF_LARGE <- data.frame(
  response_id = 1:100,
  q1_shared = sample(c(0, 1), 100, replace = TRUE),
  q4_shared = sample(c(0, 1), 100, replace = TRUE),
  q11_shared = sample(c(0, 1), 100, replace = TRUE),
  factor_variable = sample(c("Group A", "Group B", "Group C"), 100, replace = TRUE)
)

QUESTIONS <- c("q1_shared", "q4_shared", "q11_shared")

generate_multiple_tables_and_plots_by_factor(DF_LARGE, "factor_variable", QUESTIONS)